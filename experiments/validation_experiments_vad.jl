...
# ╔═╡ 32e0819c-481c-4910-934b-f21bff2d30ae
md"""
noise variance: $(@bind noise_var Slider(0:0.01:1; default=0.5, show_value=true))
"""
...
begin
	...
	signal_noisy = signal + randn(MersenneTwister(123), length(signal))*sqrt(noise_var)
	
	# plot signal
	plt.figure(figsize=(10, 4))
	plt.plot(collect(1:length(signal))./ 16000, signal_noisy, label="noisy speech")
	plt.plot(collect(1:length(signal))./ 16000, signal, label="clean speech")
	plt.legend()
	plt.grid()
	plt.xlabel("time [sec]")
	plt.gcf()
...
@model function model_VAD(ρ, σ2, zp, T)
	...
	# specify variables that are updated online
	μ_s = datavar(Float64)
	Σ_s = datavar(Float64)
    ...
    y ~ NormalMeanPrecision(x, 1/noise_var)
...
results_VAD = run_VAD(signal_noisy, ρ, σ2, [0.5, 0.5], T)

# ╔═╡ f76a4388-87bf-4c68-8c2c-00f0d7b53102
begin
	# plot signal
	fig, ax1 = plt.subplots(figsize=(10,4))
	plt.plot(collect(1:length(signal_noisy))./ 16000, signal_noisy)
	plt.plot(collect(1:length(signal))./ 16000, signal)
	plt.grid()
	plt.xlabel("time [sec]")
	plt.ylabel("signal")
	ax2 = ax1.twinx()
	plt.plot(collect(1:length(signal_noisy)-100)./ 16000, -(conv(argmax.(probvec.(results_VAD.history[:z_new])), 1/100*ones(100))[100:end-100]).+2, c="orange")
...
begin
	results_VAD_plot = run_VAD(signal_noisy, 0.6, 0.45, [0.5, 0.5], [1-1e-5 1e-5; 1e-5 1-1e-5])
	...
	fig_tikz = @pgf GroupPlot(
		...
		Plot({
			...
			Table(
				collect(1:length(signal_noisy))./ 16000,
				signal_noisy
			)
    	),
	    ...
	    Plot({ 
	        ...
	        Table(collect(1:length(signal))./ 16000, 
				-100*(argmax.(probvec.(results_VAD_plot.history[:z_new]))).+200)
	    ),
			
	)
end

# ╔═╡ d490eaae-4448-49f3-9edc-56fca8f887ae
begin
	pgfsave("../exports/validation_experiments_vad.tikz", fig_tikz)
	pgfsave("../exports/validation_experiments_vad.png", fig_tikz)
	pgfsave("../exports/validation_experiments_vad.pdf", fig_tikz)
end
...
julia_version = "1.8.1"
manifest_format = "2.0"
project_hash = "b749ab1f9d56d13dcf041d459059b194280bdde4"
